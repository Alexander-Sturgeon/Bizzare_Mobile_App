...
class UpdateListingPageState extends State<UpdateListingPage>{
  ...
  static const String placeholderImage = 'assets/images/placeholderListingImage.jpg';
  ...
  final FormGroup frmListing = FormGroup({
    'title': FormControl<String>(validators: [Validators.required]), 'description': FormControl<String>(validators: [Validators.required]),
    'price': FormControl<String>(validators: [Validators.required, Validators.number()]),
  ...
  Widget build(BuildContext context){
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: AppColors.background, leading: IconButton(
          icon: Icon(Icons.arrow_back, 
          color: AppColors.primary), 
          onPressed: () => Navigator.pop(context),
        ),
        title: Text(//main title text for the page.
          'Edit Listing',
          style: TextStyle(
        ...
        actions: [
          TextButton(//this is the button that actually calls the helper method that accesses the db via DBHelper found below. 
            onPressed: () => _updateListing(context), child: Text(
              'Save',
              style: TextStyle(
      ...
      child: 
      ReactiveForm(//reactive form is what basically makes form group available to the rest of the tree underneath it. It's essentially a way to make things cleaner; seperating the form data and ruleset from the widgets. Without it we would have needed 5 controllers with 5 rulesets and 5 things to dispose. 
        ...
        child: ListView(
          children: [
            ...
            SizedBox(height: 20),
            SizedBox(
              child: ElevatedButton(//This is what calls the helper method that accesses the db helper function via DBHelper and actually deletes the listing. 
  ...
  Future<void> _updateListing(BuildContext context) async {
    ...
    if(frmListing.valid){
      ...
      final DBResult result = await DBHelper.dbListing.updateListing(listingId!, listing);//await that calls the updateListing method to actually update the corrosponding listing in the db. 
