...
class DBHelper {
  ...
  Future<DBResult> insertListing(Map<String, dynamic> listingDetailsRow) async {
    ...
    try {
      ...
      if (listingId > 0) {
        return DBResult(
          isSuccess: true,
          message: "${listingDetailsRow['title']} was added to your listings",
          listingList: [],
        );
      } else {
        return DBResult(
  ...
  Future<DBResult> deleteListing(int id) async {
    ...
    try {
      ...
      return DBResult(
        ...
        message: "Error: $e",
        listingList: [],
      ); //returns if there is an issue encountered during deletion
    }
  }
  ...
  Future<DBResult> updateListing(
    int id,
    Map<String, dynamic> listingDetailsRow,
  ) async {
    final db = await dbListing.listingDatabase;
    try {
